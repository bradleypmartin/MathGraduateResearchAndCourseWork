function [bigLmatrix bigDampMatrix initdatavec xnodes ynodes lambdaMuVec tripleVec] = ...
ExeprepRBF1stOrderEWEInline6(N,GAshp,stencilsize,polydegree,...
    ...
    const1,const2,const3,yLI,yUI,queryFactor)

% Edit 6: adding support for 2 curved interfaces
% Edit 1: changing c2Vec to lambdaMuVec; c1s, c2s to lambdaMus
% Edit 2: switching to 1st-order EWE
% Edit 4: switching continuity matrices (u,v) to EWE form
% Edit 5: optimizing code a bit
% 10k nodes before: 86 sec.
...
if size(xnodes,1) == 1
    ...
    if intWrapFlag == 1
       ...
       xnodes(1+2*numIntNodes:3*numIntNodes,1) = xgrid + sin(theta2)*hInt*0.5;
       ynodes(1+2*numIntNodes:3*numIntNodes,1) = (curvedinterface2(xgrid,yUI)-0.5*hInt*cos(theta2));
       xnodes(1+3*numIntNodes:4*numIntNodes,1) = xgrid - sin(theta2)*hInt*0.5;
       ynodes(1+3*numIntNodes:4*numIntNodes,1) = (curvedinterface2(xgrid,yUI)+0.5*hInt*cos(theta2));
       
       xnodes(1+4*numIntNodes:5*numIntNodes,1) = xgrid + sin(theta1)*hInt*(0.5+3^0.5/2)+hInt*0.5;
       ynodes(1+4*numIntNodes:5*numIntNodes,1) = (curvedinterface1(xgrid,yLI)-(0.5+3^0.5/2)*cos(theta1)*hInt);
       xnodes(1+5*numIntNodes:6*numIntNodes,1) = xgrid - sin(theta1)*hInt*(0.5+3^0.5/2)+hInt*0.5;
       ynodes(1+5*numIntNodes:6*numIntNodes,1) = (curvedinterface1(xgrid,yLI)+(0.5+3^0.5/2)*hInt*cos(theta1));
       xnodes(1+6*numIntNodes:7*numIntNodes,1) = xgrid + sin(theta2)*hInt*(0.5+3^0.5/2)+hInt*0.5;
       ynodes(1+6*numIntNodes:7*numIntNodes,1) = (curvedinterface2(xgrid,yUI)-(0.5+3^0.5/2)*hInt*cos(theta2));
       xnodes(1+7*numIntNodes:8*numIntNodes,1) = xgrid - sin(theta2)*hInt*(0.5+3^0.5/2)+hInt*0.5;
       ynodes(1+7*numIntNodes:8*numIntNodes,1) = (curvedinterface2(xgrid,yUI)+(0.5+3^0.5/2)*hInt*cos(theta2));
       
       xnodes(1+8*numIntNodes:9*numIntNodes,1) = xgrid + sin(theta1)*hInt*(0.5+3^0.5);
       ynodes(1+8*numIntNodes:9*numIntNodes,1) = (curvedinterface1(xgrid,yLI)-(0.5+3^0.5)*cos(theta1)*hInt);
       xnodes(1+9*numIntNodes:10*numIntNodes,1) = xgrid - sin(theta1)*hInt*(0.5+3^0.5);
       ynodes(1+9*numIntNodes:10*numIntNodes,1) = (curvedinterface1(xgrid,yLI)+(0.5+3^0.5)*hInt*cos(theta1));
    ...
    for movement_iteration = 1:totalmovements
        ...
        numIntNodes,intWrapFlag,yLI,yUI);
...
lambdaMuMatrix = sparse(1:N,1:N,lambdaMuVec,N,N);
...
[bigLmatrix tripleVec] = ...
    ...
    stencilsizeInt,naiveFlag,1,yLI,yUI,queryFactor,0);
%
bigDampMatrix = ...
    createRBFLoperator1(xnodes,ynodes,xlb,xub,ylb,yub,GAshp,...
    stencilsize,polydegree,seqVec1,seqVec2,...
    lambdaMu1,rho1,lambdaMu2,rho2,polydegreeInt,...
    stencilsizeInt,naiveFlag,dampLk,yLI,yUI,queryFactor,1);
...
function [xnew ynew] = mos2dsqperiodic7(x,y,delta,nnn,xlb,xub,ylb,yub,numIntNodes,intWrapFlag,yLI,yUI)
...
function [sparseLmatrix tripleVec] = ...
    ...
    polydegree,seqVec1,seqVec2,lambdaMu1,rho1,lambdaMu2,rho2,polydegreeInt,...
    stencilsizeInt,naiveFlag,Lk,yLI,yUI,queryFactor,hyperFlag)
...
zerosStencil = zeros(stencilsize,1);
stencilsizeTemp = stencilsize;
...
parfor m = 1:N1
    ...
    %%% START HERE AFTER LUNCH - recall new functions for aughyp, augdx/y %%%
        ...
        [c1Matrix c2Matrix c3Matrix c4Matrix pFuncsUV pFuncsFGH nullMatrix1 nullMatrix2] = ...
            ...
            xeval,yeval,polydegreeInt,lambdaMu1,rho1,lambdaMu2,rho2,xClosest,Lk,yLI,yUI,tripleFlag,1/normfactorint(m,1),evalZone,zoneWidth);
        ...
        if hyperFlag > 0
            ...
            % u hyp
            sparseiL1(:,m) = m*onesStencil+0*N1;
            sparsejL1(:,m) = tempidxstack'+0*N1;
            sparsewL1(:,m) = (-1)^(Lk+1)*row1w(1,1:stencilsizeTemp)';
            
            sparseiL2(:,m) = m*onesStencil+0*N1;
            sparsejL2(:,m) = tempidxstack'+1*N1;
            sparsewL2(:,m) = (-1)^(Lk+1)*row1w(1,1+stencilsizeTemp:2*stencilsizeTemp)';
            
            % v hyp
            sparseiL3(:,m) = m*onesStencil+1*N1;
            sparsejL3(:,m) = tempidxstack'+0*N1;
            sparsewL3(:,m) = (-1)^(Lk+1)*row2w(1,1:stencilsizeTemp)';
            
            sparseiL4(:,m) = m*onesStencil+1*N1;
            sparsejL4(:,m) = tempidxstack'+1*N1;
            sparsewL4(:,m) = (-1)^(Lk+1)*row2w(1,1+stencilsizeTemp:2*stencilsizeTemp)';
            
            % f hyp
            sparseiL5(:,m) = m*onesStencil+2*N1;
            sparsejL5(:,m) = tempidxstack'+2*N1;
            sparsewL5(:,m) = (-1)^(Lk+1)*row3w(1,1:stencilsizeTemp)';
            
            sparseiL6(:,m) = m*onesStencil+2*N1;
            sparsejL6(:,m) = tempidxstack'+3*N1;
            sparsewL6(:,m) = (-1)^(Lk+1)*row3w(1,1+stencilsizeTemp:2*stencilsizeTemp)';
            
            sparseiL7(:,m) = m*onesStencil+2*N1;
            sparsejL7(:,m) = tempidxstack'+4*N1;
            sparsewL7(:,m) = (-1)^(Lk+1)*row3w(1,1+2*stencilsizeTemp:3*stencilsizeTemp)';
            
            % g hyp
            sparseiL8(:,m) = m*onesStencil+3*N1;
            sparsejL8(:,m) = tempidxstack'+2*N1;
            sparsewL8(:,m) = (-1)^(Lk+1)*row4w(1,1:stencilsizeTemp)';
            
            sparseiL9(:,m) = m*onesStencil+3*N1;
            sparsejL9(:,m) = tempidxstack'+3*N1;
            sparsewL9(:,m) = (-1)^(Lk+1)*row4w(1,1+stencilsizeTemp:2*stencilsizeTemp)';
            
            sparseiL10(:,m) = m*onesStencil+3*N1;
            sparsejL10(:,m) = tempidxstack'+4*N1;
            sparsewL10(:,m) = (-1)^(Lk+1)*row4w(1,1+2*stencilsizeTemp:3*stencilsizeTemp)';
            
            % h hyp
            sparseiL11(:,m) = m*onesStencil+4*N1;
            sparsejL11(:,m) = tempidxstack'+2*N1;
            sparsewL11(:,m) = (-1)^(Lk+1)*row5w(1,1:stencilsizeTemp)';
            
            sparseiL12(:,m) = m*onesStencil+4*N1;
            sparsejL12(:,m) = tempidxstack'+3*N1;
            sparsewL12(:,m) = (-1)^(Lk+1)*row5w(1,1+stencilsizeTemp:2*stencilsizeTemp)';
            
            sparseiL13(:,m) = m*onesStencil+4*N1;
            sparsejL13(:,m) = tempidxstack'+4*N1;
            sparsewL13(:,m) = (-1)^(Lk+1)*row5w(1,1+2*stencilsizeTemp:3*stencilsizeTemp)';
            ...
            sparseiL1(:,m) = m*onesStencil+0*N1;
            sparsejL1(:,m) = tempidxstack'+2*N1;
            sparsewL1(:,m) = row1w(1,1:stencilsizeTemp)';
            
            sparseiL2(:,m) = m*onesStencil+0*N1;
            sparsejL2(:,m) = tempidxstack'+3*N1;
            sparsewL2(:,m) = row1w(1,1+stencilsizeTemp:2*stencilsizeTemp)';
            
            sparseiL3(:,m) = m*onesStencil+0*N1;
            sparsejL3(:,m) = tempidxstack'+4*N1;
            sparsewL3(:,m) = row1w(1,1+2*stencilsizeTemp:3*stencilsizeTemp)';
            
            % Row2 (vt = gx/rho + hy/rho)
            sparseiL4(:,m) = m*onesStencil+1*N1;
            sparsejL4(:,m) = tempidxstack'+2*N1;
            sparsewL4(:,m) = row2w(1,1:stencilsizeTemp)';
            
            sparseiL5(:,m) = m*onesStencil+1*N1;
            sparsejL5(:,m) = tempidxstack'+3*N1;
            sparsewL5(:,m) = row2w(1,1+stencilsizeTemp:2*stencilsizeTemp)';
            
            sparseiL6(:,m) = m*onesStencil+1*N1;
            sparsejL6(:,m) = tempidxstack'+4*N1;
            sparsewL6(:,m) = row2w(1,1+2*stencilsizeTemp:3*stencilsizeTemp)';
            
            % Row3 (ft = 3*lambdamu*ux + lambdamu*vy)
            sparseiL7(:,m) = m*onesStencil+2*N1;
            sparsejL7(:,m) = tempidxstack'+0*N1;
            sparsewL7(:,m) = row3w(1,1:stencilsizeTemp)';
            
            sparseiL8(:,m) = m*onesStencil+2*N1;
            sparsejL8(:,m) = tempidxstack'+1*N1;
            sparsewL8(:,m) = row3w(1,1+stencilsizeTemp:2*stencilsizeTemp)';
            
            % Row4 (gt = lambdamu*uy + lambdamu*vx)
            sparseiL9(:,m) = m*onesStencil+3*N1;
            sparsejL9(:,m) = tempidxstack'+0*N1;
            sparsewL9(:,m) = row4w(1,1:stencilsizeTemp)';
            
            sparseiL10(:,m) = m*onesStencil+3*N1;
            sparsejL10(:,m) = tempidxstack'+1*N1;
            sparsewL10(:,m) = row4w(1,1+stencilsizeTemp:2*stencilsizeTemp)';
            
            % Row5 (ht = lambdamu*ux + 3*lambdamu*vy)
            sparseiL11(:,m) = m*onesStencil+4*N1;
            sparsejL11(:,m) = tempidxstack'+0*N1;
            sparsewL11(:,m) = row5w(1,1:stencilsizeTemp)';
            
            sparseiL12(:,m) = m*onesStencil+4*N1;
            sparsejL12(:,m) = tempidxstack'+1*N1;
            sparsewL12(:,m) = row5w(1,1+stencilsizeTemp:2*stencilsizeTemp)';
...
function [c1Matrix c2Matrix c3Matrix c4Matrix pFuncsUV pFuncsFGH nullMatrix1 nullMatrix2] = ...
    ...
    polydegreeInt,lambdaMu1,rho1,lambdaMu2,rho2,xClosest,Lk,yLI,yUI,tripleFlag,normfactor,evalZone,zoneWidth)
