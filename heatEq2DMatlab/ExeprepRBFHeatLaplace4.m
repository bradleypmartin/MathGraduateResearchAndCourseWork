...
% Edit 4 3/8 allowing switch between curved and horiz. int's
...
hApprox = (xub-xlb)/N^0.5;
boundVec = (abs(xub-xnodes) < (3*hApprox)) + ...
    (abs(xlb-xnodes) < (3*hApprox)) + ...
    (abs(yub-ynodes) < (3*hApprox)) + ...
    (abs(ylb-ynodes) < (3*hApprox));
...
[preLmatrix tripleVec] = ...
    ...
    boundVec,stencilsizeBound,polydegreeBound);
%{
bigDampMatrix = ...
    createRBFLoperator1(xnodes,ynodes,xlb,xub,ylb,yub,GAshp,...
    stencilsize,polydegree,seqVec1,seqVec2,...
...
for m = 1:(N1-2*numIntNodes)
    ...
    if seqFlag == 0
        ...
        if boundVec(m,1) > 0
            stencilsizeTemp = stencilsizeBound;
            polydegreeTemp = polydegreeBound;
        else
            stencilsizeTemp = stencilsize;
            polydegreeTemp = polydegree;
        end
        
        A = ones(stencilsizeTemp,stencilsizeTemp);
        RHS = zeros(stencilsizeTemp,1);
        
        xpositions = zeros(1,stencilsizeTemp);
        ypositions = zeros(1,stencilsizeTemp);
        
        for n = 1:stencilsizeTemp
        ...
        for i = 1:stencilsizeTemp        % creating the A matrix.
            for j = 1:stencilsizeTemp
                A(i,j) = ...
                    exp(-shp^2*...
        ...
        hyperviscmatrixtemp = augLwithpolys1(A,RHS,xpositions,ypositions,...
            xpositions(1,1),ypositions(1,1),polydegreeTemp,1/normfactorint(m,1),Lk);
