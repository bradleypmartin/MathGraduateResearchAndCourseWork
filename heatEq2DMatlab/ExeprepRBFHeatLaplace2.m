...
% Edit 2 2/16 closed boundary all around
...
if size(xnodes,1) == 1
    ...
    xnodes(N-2*numIntNodes+1:N-numIntNodes,1) = 1-xInt';
    xnodes(N-3*numIntNodes+1:N-2*numIntNodes,1) = ones(numIntNodes,1);
    xnodes(N-4*numIntNodes+1:N-3*numIntNodes,1) = zeros(numIntNodes,1);
    ynodes(N-numIntNodes+1:N,1) = ones(numIntNodes,1);
    ynodes(N-2*numIntNodes+1:N-numIntNodes,1) = zeros(numIntNodes,1);
    ynodes(N-3*numIntNodes+1:N-2*numIntNodes,1) = 1-xInt';
    ynodes(N-4*numIntNodes+1:N-3*numIntNodes,1) = xInt';
...
for m = (startM+1):(size(x,1)-4*numIntNodes)
...
for k = 1:size(xnew,1)
    ...
    if k > startM && (k <= (size(x,1)-4*numIntNodes))
...
for m = 1:(N1-4*numIntNodes)
    ...
    if seqFlag == 0
        ...
        if ypositionsInt(1,1) < (curvedinterface1(xpositionsInt(1,1),yLI)+curvedinterface2(xpositionsInt(1,1),yUI))/2
            [xClosest theta] = pointFinder1(xpositionsInt(1,1),ypositionsInt(1,1));           
            ...
            [xClosest theta] = pointFinder2(xpositionsInt(1,1),ypositionsInt(1,1));   
        ...
        [c1Matrix c2Matrix c3Matrix c4Matrix nullMatrix1 nullMatrix2] = ...
            ...
            xeval,yeval,polydegreeInt,c1,rho1,c2,rho2,xClosest,Lk,yLI,yUI,tripleFlag,1/normfactorint(m,1),evalZone);
...
function [c1Matrix c2Matrix c3Matrix c4Matrix nullMatrix1 nullMatrix2] = ...
    ...
    polydegreeInt,c1,rho1,c2,rho2,xClosest,Lk,yLI,yUI,tripleFlag,normfactor,evalZone)
...
function [xClosest theta] = pointFinder1(x,y)
    ...
    xClosest = [xClosest; x]; yClosest = [yClosest; curvedinterface1(x)];
    theta = []; theta = [theta; 0];
    
    for k = 2:20
        [yeval yprime] = curvedinterface1(xClosest(k-1,1));
        ...
        yClosest = [yClosest; curvedinterface1(newVec3(1,1))];  
    end
  %}
end

function [xClosest theta] = pointFinder2(x,y)
    xClosest = x; theta = 0;
    %{
    xClosest = []; yClosest = [];
    
    xClosest = [xClosest; x]; yClosest = [yClosest; curvedinterface2(x)];
    theta = []; theta = [theta; 0];
    
    for k = 2:20
        [yeval yprime] = curvedinterface2(xClosest(k-1,1));
        ...
        xClosest = [xClosest; newVec3(1,1)];
        yClosest = [yClosest; curvedinterface2(newVec3(1,1))];  
    end
    %}
end